...
5 个包在 `packages/*` 下，根包 `@pawover/kit` 通过 `package.json#exports` 重新导出所有子包：
...
**因此发布根包前必须先发布 5 个子包**（尤其 `@pawover/kit-types`、`@pawover/kit-eslint-rules` 首次发布）。`scripts/verify-release.mjs` 硬校验「子包发布 ⇒ 根包必发」，防止根包落后。

根包 `build` 任务（`scripts/sync-entry.ts`）在 turbo 流水线中同步 `entry/metadata.json` 与 `entry/hooks-metadata.json`（见「构建流水线」）。改根包 exports / entry 后需 `pnpm build` 再跑 `check:eslint`（entry 与根 scripts 已在 eslint ignores）。
...
- 注意 `packages/*/tsconfig.json` 无 paths，IDE 中包间导入经 workspace 链接解析到 `dist` d.ts（构建后）。
